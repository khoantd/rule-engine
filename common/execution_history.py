...
class ExecutionHistory:
    ...
    def __init__(self, max_records: int = 10000, retention_days: int = 30):
        """
    ...
    def log_execution(
        ...
        try:
            with get_db_session() as session:
                ...
                logger.debug("Execution logged to database", execution_id=execution_id)
        except Exception as db_error:
            # Don't fail execution if DB logging fails
            logger.warning(
    ...
    def get_execution(self, execution_id: str) -> Optional[RuleExecutionRecord]:
        """
    ...
    def get_by_correlation_id(self, correlation_id: str) -> List[RuleExecutionRecord]:
        """
    ...
    def get_by_rule_name(
        self, rule_name: str, limit: Optional[int] = None
    ...
    def cleanup(self, before_date: Optional[datetime] = None) -> int:
        ...
        for correlation_id in list(self._by_correlation_id.keys()):
            self._by_correlation_id[correlation_id] = [
                r
                for r in self._by_correlation_id[correlation_id]
