...
def validate_file_path(
    file_path: str, allowed_base: Optional[str] = None, must_exist: bool = False
) -> Path:
    """
    ...
    if allowed_base:
        try:
            ...
            if not path.is_relative_to(base_path):
                logger.error(
                    "Path outside allowed base", file_path=file_path, allowed_base=allowed_base
                )
                raise SecurityError(
...
def validate_config_secrets(config_dict: dict, required_secrets: Optional[list] = None) -> None:
    ...
    for key, value in config_dict.items():
        ...
        if is_secret_key and value:
            ...
            # Skip if value matches the conventional env var (intentional injection)
            env_key = key.upper().replace("-", "_")
            env_val = os.getenv(env_key)
            if env_val is not None and env_val == value_str:
                continue
...
def validate_s3_key(s3_key: str) -> None:
    ...
    if not s3_key or not isinstance(s3_key, str):
        raise SecurityError(
            f"Invalid S3 key: {s3_key}", error_code="INVALID_S3_KEY", context={"s3_key": s3_key}
        )
