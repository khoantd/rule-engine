...
class FileConfigRepository(ConfigRepository):
    ...
    def read_rules_set(self, source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(parsed_data, list):
                logger.info("Rules set loaded successfully", 
                           source=source, rules_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected rules set format", source=source, 
                              data_type=type(parsed_data).__name__)
                return []
        except Exception as e:
            logger.error("Failed to read rules set from file", source=source, 
    ...
    def _read_rules_from_dmn(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Rules loaded from DMN file", 
                       file_path=file_path, 
                       rules_count=len(rules))
            
            return rules
        except Exception as e:
            logger.error("Failed to read rules from DMN file", 
                        file_path=file_path, 
                        error=str(e), 
                        exc_info=True)
            raise ConfigurationError(
    ...
    def read_conditions_set(self, source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(parsed_data, list):
                logger.info("Conditions set loaded successfully", 
                           source=source, conditions_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected conditions set format", source=source,
                              data_type=type(parsed_data).__name__)
                return []
        except Exception as e:
            logger.error("Failed to read conditions set from file", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
        ...
        try:
            ...
            if isinstance(parsed_data, dict):
                logger.info("Patterns loaded successfully", 
                           source=source, patterns_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected patterns format", source=source,
                              data_type=type(parsed_data).__name__)
                return {}
        except Exception as e:
            logger.error("Failed to read patterns from file", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
    ...
    def read_json(self, source: str) -> Union[Dict[str, Any], List[Any]]:
        ...
        try:
            ...
            logger.error("Failed to read JSON/DMN from file", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
...
class S3ConfigRepository(ConfigRepository):
    ...
    def _read_from_s3(self, key: str) -> str:
        ...
        try:
            ...
            logger.error("Failed to read from S3", bucket=self.bucket, key=key,
                        error=str(e), exc_info=True)
            raise StorageError(
    ...
    def read_rules_set(self, source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(parsed_data, list):
                logger.info("Rules set loaded from S3", 
                           source=source, rules_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected rules set format", source=source,
            ...
            logger.error("Failed to read rules set from S3", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
    ...
    def read_conditions_set(self, source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(parsed_data, list):
                logger.info("Conditions set loaded from S3", 
                           source=source, conditions_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected conditions set format", source=source,
            ...
            logger.error("Failed to read conditions set from S3", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
    ...
    def read_patterns(self, source: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(parsed_data, dict):
                logger.info("Patterns loaded from S3", 
                           source=source, patterns_count=len(parsed_data))
                return parsed_data
            else:
                logger.warning("Unexpected patterns format", source=source,
            ...
            logger.error("Failed to read patterns from S3", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
    ...
    def read_json(self, source: str) -> Union[Dict[str, Any], List[Any]]:
        ...
        try:
            ...
            logger.error("Failed to read JSON from S3", source=source,
                        error=str(e), exc_info=True)
            raise ConfigurationError(
...
def get_config_repository() -> ConfigRepository:
    ...
    If S3 bucket is configured, returns S3 repository, otherwise file repository.
    
    Returns:
        ConfigRepository instance (FileConfigRepository or S3ConfigRepository)
    """
    global _repository
...
def set_config_repository(repository: ConfigRepository) -> None:
    ...
    logger.debug("Configuration repository set", 
