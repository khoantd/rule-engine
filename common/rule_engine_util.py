...
def rules_set_exec(
    rules_set: List[Dict[str, Any]], conditionss_set: List[Condition]
) -> List[Dict[str, Any]]:
    """
...
def rule_prepare(
    ...
    - Combining multiple conditions with logical operators (for complex rules).
      Complex ``conditions.items`` may list condition IDs (strings) and/or inline
      dicts ``{"attribute", "equation"|"condition", "constant"}`` (no catalog lookup).
    ...
    try:
        ...
        if isinstance(rule, dict):
            try:
                ...
                logger.error(
                    "Invalid rule structure", rule_name=rule_name, error=str(e), exc_info=True
                )
                raise RuleCompilationError(
        ...
        if tmp_rule.type == "complex":
            ...
            # Build condition strings (each item: condition_id str, or inline dict)
            for condition_index in range(len(tmp_conditions)):
                entry = tmp_conditions[condition_index]
                ...
                if not isinstance(entry, str):
                    raise RuleCompilationError(
                        _format_rule_error_message(
                            rule_name,
                            "complex rule conditions.items entries must be condition id strings "
                            "or inline dicts with attribute, equation (or condition), and constant",
                            conditions_info="items[{!r}]={!r}".format(condition_index, entry),
                        ),
                        error_code="RULE_INVALID_COMPLEX_ITEM",
                        context={
                            "rule_name": rule_name,
                ...
                if not condition_found:
                    logger.warning(
                        "Condition not found in conditions set",
                        rule_name=rule_name,
                        condition_id=condition_id,
                    )
            ...
            if tmp_logical_operator == "nothing":
                mode_raw = tmp_rule.conditions.get("mode")
                cond_info = f"mode={mode_raw!r}, condition_ids={tmp_conditions!r}"
                raise RuleCompilationError(
                    _format_rule_error_message(
                        rule_name,
                        "invalid conditions.mode for complex rule; "
                        "use 'and', 'or', 'inclusive', or 'exclusive'",
                        conditions_info=cond_info,
                    ),
            ...
            logger.debug(
                "Simple rule condition prepared",
                rule_name=rule_name,
                condition_str=tmp_cond_concated_str,
            )
        ...
        logger.debug(
            "Rule prepared successfully",
            rule_name=rule_name,
            priority=tmp_rule.priority,
            condition_length=len(tmp_cond_concated_str),
        )
        ...
        logger.error(
            "Unexpected error preparing rule", rule_name=rule_name, error=str(error), exc_info=True
        )
        raise RuleCompilationError(
...
def _rule_engine_string_operand(constant: str) -> str:
    """
    Format *constant* as a ``rule_engine.Rule`` operand.

    Arrays/objects (``[...]``, ``{...}``) and already-quoted string literals are passed
    through. Numeric strings are left bare; other scalars are escaped and double-quoted.
    """
    if constant is None:
        return '""'
    s = str(constant).strip()
    if not s:
        return '""'
...
def format_rule_engine_condition_clause(
    attribute: str,
    equation: str,
    constant: str,
    *,
    ...
    """
    Build one boolean clause for ``rule_engine.Rule`` from attribute / operator / constant.

    The underlying engine's ``in`` operator uses Python containment: for strings,
    ``needle in haystack`` is expressed as ``needle in attribute`` (needle first).

    Args:
        attribute: Input data field name (left-hand side for most operators).
        equation: Operator name from DB/API (e.g. ``equal``, ``contains``, ``in``).
        constant: Right-hand value or composite literal (e.g. JSON-like list for ``in``).
        rule_name: Rule name for error messages.

    Returns:
        A fragment suitable for parsing by ``rule_engine.Rule``. Regex operators
        (``regex``, ``regex_match``, etc.) embed the pattern as a quoted string so
        metacharacters like ``^`` are not parsed as expression operators.

    Raises:
        RuleCompilationError: If the attribute is empty where required, or the operator
            is not supported.
    """
    attr = (attribute or "").strip()
    const_raw = "" if constant is None else str(constant)
    eq_key = (equation or "").strip().lower()
    ...
    if eq_key == "contains":
        if not attr:
            raise RuleCompilationError(
                _format_rule_error_message(
                    rule_name,
                    "empty attribute for contains condition",
                    conditions_info="contains",
                ),
                error_code="CONDITION_EMPTY",
                context={"rule_name": rule_name, "attribute": attribute, "equation": equation},
            )
        return "{} in {}".format(_rule_engine_string_operand(const_raw), attr)
    ...
    if eq_key == "in":
        if not attr:
            raise RuleCompilationError(
                _format_rule_error_message(
                    rule_name,
                    "empty attribute for in condition",
                    conditions_info="in",
                ),
                error_code="CONDITION_EMPTY",
                context={"rule_name": rule_name, "attribute": attribute, "equation": equation},
            )
        return "{} in {}".format(attr, _rule_engine_string_operand(const_raw))
    ...
    if eq_key == "not_in":
        if not attr:
            raise RuleCompilationError(
                _format_rule_error_message(
                    rule_name,
                    "empty attribute for not_in condition",
                    conditions_info="not_in",
                ),
                error_code="CONDITION_EMPTY",
                context={"rule_name": rule_name, "attribute": attribute, "equation": equation},
            )
        return "not ({} in {})".format(attr, _rule_engine_string_operand(const_raw))

    op = equation_operators(equation)
    if op == "nothing":
        raise RuleCompilationError(
            _format_rule_error_message(
                rule_name,
                "unsupported equation operator {!r}".format(equation),
                conditions_info="equation={!r}".format(equation),
            ),
            error_code="CONDITION_OPERATOR_UNSUPPORTED",
            context={
                "rule_name": rule_name,
                "attribute": attribute,
                "equation": equation,
...
def condition_setup(rule: Dict[str, Any]) -> str:
    ...
    Raises:
        ...
        RuleCompilationError: If equation operator is invalid or unsupported

    Example:
        >>> rule = {'attribute': 'status', 'condition': 'equal', 'constant': '"active"'}
        >>> condition_setup(rule)
        'status == "active"'
    """
    return format_rule_engine_condition_clause(
        str(rule["attribute"]),
        str(rule["condition"]),
        str(rule["constant"]),
        rule_name=str(rule.get("rule_name", rule.get("rulename", "unknown"))),
    )
...
def rules_set_from_s3_read(config_file: str) -> List[Dict[str, Any]]:
    ...
    parsed_data_main_node = parse_json_v2("$.rules_set", json.loads(cfg_content))
    return parsed_data_main_node
...
def rule_actions_from_S3_read(config_file: str) -> Dict[str, Any]:
    ...
    """
    cfg_content = config_file_read("S3", config_file)
    parsed_data_main_node = parse_json_v2("$.patterns", json.loads(cfg_content))
    return parsed_data_main_node


def find_action_recommendation(actions_list: Dict[str, Any], data: str) -> Optional[str]:
    """
    ...
    if not isinstance(actions_list, dict):
        logger.error(
            "Actions list must be a dictionary", actions_list_type=type(actions_list).__name__
        )
...
def rule_run(rule: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            ...
            rule_point=rule.get("rule_point"),
        )
        if rule_matched:
            ...
            try:
                ...
                logger.warning(
                    "Failed to convert rule_point or weight",
                    rule_id=rule_id,
                    rule_point=rule_point_value,
                    weight=weight_value,
        ...
        missing_symbol = (
            str(symbol_error).split("'")[1] if "'" in str(symbol_error) else str(symbol_error)
        )
        logger.warning(
            "Rule evaluation failed - symbol not found",
            rule_id=rule_id,
    ...
    return {"action_result": tmp_action, "rule_point": tmp_point, "weight": tmp_weight}
