...
def rules_exec(rules_list, data):
    ...
    tmp_action = find_action_recommendation(rule_actions_read(
        "config/rules_config.json"), tmp_str)
    rs = {
        "total_points": sum,
...
if __name__ == "__main__":
    logger.info("Starting main_rule_exec_v1.py execution")
    rule_testing()
    logger.info("Loading rules configuration", config_file="config/rules_config.json")
    rules_list = rules_setup(rules_set_read("config/rules_config.json"))
    data = {
        'title': 'Superman',
        'publisher': 'DC',
        'issue': 40
