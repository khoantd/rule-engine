...
class TestRulePrepare:
    ...
    def test_rule_prepare_condition_not_found(self, sample_condition_objects):
        ...
        assert (
            "NONEXISTENT" in msg or "condition" in msg.lower()
    ...
    def test_rule_prepare_flat_format_empty_attribute_raises_condition_empty(
        ...
        assert "Rule1" in msg, "Error message should include rule name"
        assert (
            "conditions:" in msg or "attribute=" in msg
...
class TestRulesSetSetup:
    ...
    def test_rules_set_setup_success(
        self, mock_get_cache, mock_conditions_load, sample_condition_objects
    ):
...
class TestRulesSetExec:
    ...
    def test_rules_set_exec_skips_degenerate_flat_rule(self, sample_condition_objects):
        """Degenerate DB-style flat rows are skipped with a warning (like CONDITION_EMPTY)."""
        rules_set = [
            {
                "rulename": "Rule1",
                "type": "simple",
    ...
    def test_rule_prepare_prefers_structured_when_flat_columns_present(
        self, sample_condition_objects
    ):
        """Catalog-backed conditions win over legacy flat columns on the same dict."""
        rule = {
            "rule_name": "Hybrid",
            "type": "simple",
            "conditions": {"item": "C0001"},
        ...
        prepared = rule_prepare(sample_condition_objects, rule)
        assert prepared["rule_name"] == "Hybrid"
        assert "status" in prepared["condition"]
