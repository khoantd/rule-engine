...
def validate_input_data(data: Any) -> Dict[str, Any]:
    ...
    if data is None:
        ...
        raise DataValidationError(
            "Input data cannot be None", error_code="DATA_NONE", context={"data": data}
        )
    ...
    logger.debug(
        "Input data validated successfully", data_keys=list(data.keys()), data_length=len(data)
    )
    return data
    ...
    try:
        ...
        with metrics.timer("rule_execution", dimensions={"dry_run": str(dry_run)}):
            ...
            if not rules_list or len(rules_list) == 0:
                ...
                result = {"total_points": 0.0, "pattern_result": "", "action_recommendation": None}
                if dry_run:
                    result.update(
                        {"rule_evaluations": [], "would_match": [], "would_not_match": []}
                    )
                return result
            ...
            for rule in rules_list:
                try:
                    ...
                    if not isinstance(result, dict):
                        logger.error(
                            "Rule run returned invalid result",
                            rule_id=rule_id,
                    ...
                    try:
                        ...
                        if not isinstance(rule_point_value, (int, float, str)):
                            raise TypeError(
                                f"Invalid rule_point type: {type(rule_point_value).__name__}"
                            )
                        ...
                        logger.warning(
                            "Invalid rule_point or weight",
                            rule_id=rule_id,
                            rule_point=result.get("rule_point"),
            ...
            try:
                ...
                logger.warning(
                    "Failed to load actions configuration, continuing without action recommendation"
                )
                tmp_action = None
            ...
            if consumer_id and matched_rule_ids:
                try:
                    ...
                    logger.warning(
                        "Failed to track consumer usage", error=str(e), consumer_id=consumer_id
                    )
...
def rules_exec_by_ruleset(
    ...
    try:
        with metrics.timer(
            ...
            try:
                ...
                logger.error(
                    "Failed to load rules for ruleset",
                    ruleset_name=ruleset_name,
                    error=str(e),
                    exc_info=True,
                )
            ...
            logger.info(
                "Ruleset raw rules loaded from repository",
                ruleset_name=ruleset_name,
                correlation_id=correlation_id,
            ...
            try:
                ...
                logger.error(
                    ...
                    ruleset_name=ruleset_name,
                    correlation_id=correlation_id,
                    error_code=compile_err.error_code,
                    error=str(compile_err),
                    context=compile_err.context,
                    exc_info=True,
                )
                raise

            prepared_rules_count = len(rules_list)
            if prepared_rules_count != raw_rules_count:
                logger.warning(
                    "Prepared rule count differs from raw rules (some rules skipped during setup, "
                    "e.g. CONDITION_EMPTY or degenerate DB placeholders)",
                    ruleset_name=ruleset_name,
                    correlation_id=correlation_id,
                    raw_rules_count=raw_rules_count,
            ...
            logger.info(
                "Loaded prepared rules for ruleset execution",
                ruleset_name=ruleset_name,
                correlation_id=correlation_id,
                raw_rules_count=raw_rules_count,
                prepared_rules_count=prepared_rules_count,
            ...
            metrics.increment("rules_loaded", value=prepared_rules_count)
            ...
            tmp_action: Optional[str] = None
            action_metric_label: Optional[str] = None
            try:
                repo = get_config_repository()
                if isinstance(repo, DatabaseConfigRepository):
                    (
                        tmp_action,
                        action_metric_label,
                    ) = repo.resolve_action_recommendation_for_pattern(ruleset_name, tmp_str)
                else:
                    actions_set = actions_set_cfg_read()
                    tmp_action = find_action_recommendation(actions_set, tmp_str)
                    action_metric_label = tmp_action
            except Exception as action_error:
                logger.warning("Error getting action recommendation", error=str(action_error))
                tmp_action = None
                action_metric_label = None
            ...
            metrics.put_metric("total_points", total_points, "Count")
            metrics.increment("rules_evaluated", value=executed_rules_count)
            metrics.track_action(action_metric_label)
            metrics.track_pattern(tmp_str)
            metrics.track_points(total_points)
    ...
    except (DataValidationError, ConfigurationError, RuleEvaluationError, RuleCompilationError):
        raise
    except Exception as execution_error:
        error = str(execution_error)
        error_code = "RULES_EXEC_ERROR"
        logger.error(
            "Unexpected error in ruleset execution",
            ruleset_name=ruleset_name,
            error=error,
            exc_info=True,
        )
...
def rules_exec_batch(
    ...
    logger.info(
        "Starting batch rules execution",
        batch_id=batch_id,
    ...
    def execute_single(data_item: Dict[str, Any], index: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to execute rules for item",
                item_index=index,
                correlation_id=item_correlation_id,
                error=str(e),
            )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.error("Error getting result for item", item_index=index, error=str(e))
                    results.append(
                        {
                            "item_index": index,
                            "status": "failed",
    ...
    logger.info(
        "Batch rules execution completed",
        batch_id=batch_id,
        total_executions=len(data_list),
    ...
    return {"batch_id": batch_id, "results": results, "summary": summary, "dry_run": dry_run}


def _group_rules_by_decision(
    rules_set: List[Dict[str, Any]], decisions_metadata: List[Dict[str, Any]]
    ...
    logger.debug(
        "Grouped rules by decision", decision_counts={k: len(v) for k, v in grouped.items()}
    )
    ...
    for rule in rules:
        try:
            ...
            if rule_matched:
                ...
                logger.debug(
                    "Rule did not match in decision",
                    decision_id=decision_id,
                    decision_name=decision_metadata.get("decision_name"),
                    rule_id=rule_id,
                    condition=rule.get("condition", ""),
                ...
                if rule_outputs:
                    ...
                    for output_label, output_value in rule_outputs.items():
                        if isinstance(output_value, str):
                            evaluated_outputs[output_label] = _evaluate_feel_expression(
                                output_value, data
                            )
                        else:
                    ...
                    logger.debug(
                        "Collected multiple outputs from rule",
                        decision_id=decision_id,
                        rule_id=rule_id,
                    ...
                    if outputs_def:
                        output_label = (
                            outputs_def[0].get("label", "")
                            or outputs_def[0].get("name", "")
                            or "result"
                        )
                    else:
                        output_label = "result"
            ...
            logger.warning(
                "Error executing rule in decision",
                decision_id=decision_id,
                rule_id=rule.get("rule_name", "unknown"),
                error=str(rule_error),
            )
...
def _map_decision_outputs(
    decision_metadata: Dict[str, Any], execution_result: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    if not outputs:
        logger.warning(
            "No outputs defined for decision",
            decision_id=decision_metadata.get("decision_id"),
            decision_name=decision_metadata.get("decision_name"),
        )
        return {}
                ...
                for decision_id in execution_order:
                    ...
                    logger.info(
                        ...
                        current_data_values={
                            ...
                            if k in ["element_1", "element_2", "can", "chi"]
                        },
                    )
                    ...
                    has_matched_outputs = decision_result.get(
                        "matched_outputs_dict"
                    ) or decision_result.get("matched_outputs")
                    if has_matched_outputs:
                        mapped_outputs = _map_decision_outputs(
                            decision_metadata=decision_metadata, execution_result=decision_result
                        )
                        if mapped_outputs:
                            ...
                            logger.info(
                                "Enriched data with dependent outputs",
                                decision_id=decision_id,
                                decision_name=decision_metadata.get("decision_name"),
                ...
                for rule in prepared_rules:
                    try:
                        ...
                        if rule_matched:
                            matched_rules_count += 1
                            metrics.increment(
                                "dmn_rules_matched", dimensions={"rule_name": rule_id}
                            )
                        else:
                            metrics.increment(
                                "dmn_rules_not_matched", dimensions={"rule_name": rule_id}
                            )
                        ...
                        try:
                            ...
                            if not isinstance(weight_value, (int, float, str)):
                                raise TypeError(
                                    f"Invalid weight type: {type(weight_value).__name__}"
                                )
